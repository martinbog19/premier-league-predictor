...
class scraper:
    ...
    def __init__(self, years, code, acyear = True) :
        self.years  = years
        self.code   = code
        self.acyear = acyear
    ...
    def fit(self, windows = [1, 2, 5, 10], verbose = True) :
        ...
        for i, year in enumerate(self.years) :
            ...
            year_code = f'{year-1}-' * self.acyear + str(year)
            url = f'https://fbref.com/en/comps/{self.code}/{year_code}/schedule/'
            ...
            if soup.find('table') is not None :
                ...
                if verbose :
                    print(f'Building training data ... {year_code} ...  ({i+1}/{len(self.years)}) ...   ', end = '\r')
